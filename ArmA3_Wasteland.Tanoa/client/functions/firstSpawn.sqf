...
if (playerSide in [BLUFOR,OPFOR,INDEPENDENT] && {{_x select 0 == _uid} count pvar_teamSwitchList == 0}) then
{
	...
	{
		case BLUFOR: { "GENDARMERIE! You take care of law and order." };
		case OPFOR:  { "OPFOR" };
		case INDEPENDENT:  { "BANDITS! You spread violence and crime." };
	};

	titleText [format ["You belong to %1", _side], "PLAIN", 0.5];
};
