/*
	...
	Description: Refresh the main section of the mine menu.
	...
	Example:
		[control] call vip_sms_fnc_cl_menuUpdate;
*/
...
if (_currentSelection > -1) then {
	...
	_obj = vip_sms_var_cl_mineList select _currentSelection;
	_isTripwire = !(_obj isKindOf "Land_Map_F");
	_armed = if !(_isTripwire) then {_obj getVariable "vip_mineArm"} else {(_obj getVariable "vip_mineTripAttachedMine") getVariable "vip_mineArm"};
	_statusText = "";
	...
	switch (_armed) do {
		...
		case 0: {
			...
			if !(_isTripwire) then {(_dialog displayCtrl 2) ctrlEnable true}; //remove
			
			switch (_obj getVariable "vip_mineType") do {
