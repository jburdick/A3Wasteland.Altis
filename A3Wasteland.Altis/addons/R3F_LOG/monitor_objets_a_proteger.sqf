...
while {true} do
{
	// Pour chaque objet à protéger
	{
		...
		if (!isNull _object) then
		{
			...
			{
				// Mais que le carrier est détruit/héliporté/remorqué
				if !(alive (_object getVariable "R3F_LOG_est_transporte_par")) then
				{
					...
					else
					{
						...
						{
							_pos_respawn = getPos (_object getVariable "R3F_LOG_est_transporte_par");
						}
						else
						{
					...
					_objects_charges = (_object getVariable "R3F_LOG_est_transporte_par") getVariable ["R3F_LOG_objects_charges", []];
					if (_object in _objects_charges) then
					{
						_objects_charges = _objects_charges - [_object];
						(_object getVariable "R3F_LOG_est_transporte_par") setVariable ["R3F_LOG_objects_charges", _objects_charges, true];
					};
