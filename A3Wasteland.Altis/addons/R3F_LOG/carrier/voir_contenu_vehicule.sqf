...
private ["_carrier", "_chargement", "_chargement_precedent", "_contenu"];
...
while {!isNull _dlg_contenu_vehicule} do
{
	...
	if !([_chargement, _chargement_precedent] call BIS_fnc_areEqual) then
	{
		...
		_contenu = _carrier getVariable ["R3F_LOG_objects_charges", []];
		
		/** Liste des noms de classe des objets contenu dans le v�hicule, sans doublon */
		_tab_objects = [];
		/** Quantit� associ� (par l'index) aux noms de classe dans _tab_objects */
		_tab_quantite = [];
		/** Co�t de chargement associ� (par l'index) aux noms de classe dans _tab_objects */
		...
		for [{_i = 0}, {_i < count _contenu}, {_i = _i + 1}] do
		{
			...
			if !((typeOf _object) in _tab_objects) then
			{
				...
				if (!isNil {_object getVariable "R3F_LOG_fonctionnalites"}) then
				{
					_tab_cout_chargement pushBack (_object getVariable "R3F_LOG_fonctionnalites" select R3F_LOG_IDX_can_be_transported_cargo_cout);
				}
				else
				{
					_tab_cout_chargement pushBack (([typeOf _object] call R3F_LOG_FNCT_determiner_fonctionnalites_logistique) select R3F_LOG_IDX_can_be_transported_cargo_cout);
				};
			}
			else
			{
