...
_relockTime = [];
{
	...
	if (count ownedObjectMapMarkers > 0) then {
		...
		["Added Markers for your objects (radius 1 KM) on the map, they will be removed in 30 seconds", 5] call mf_notify_client;
		
		}else{
		
		["Nothing found. Lucky one!", 5] call mf_notify_client;
