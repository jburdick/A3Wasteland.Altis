...
{
	if (_x in R3F_LOG_objects_depl_heli_remorq_transp) exitWith {_can_be_depl_heli_remorq_transp = true;};
...
if (_can_be_depl_heli_remorq_transp) then
{
	...
	{
		_idx = R3F_LOG_classes_objects_transportables find _x;
		if (_idx != -1) exitWith
		{
...
{
	_idx = R3F_LOG_classes_carriers find _x;
	if (_idx != -1) exitWith
	{
