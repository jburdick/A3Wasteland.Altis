/*
	...
	Description: Place a mine in a 3D environment.
	
	Parameters:
		0: String; classname of mine to use.

	Returns:
		Nothing.

	Example:
		["DemoCharge_Remote_Ammo"] call vip_sms_fnc_cl_place;
*/
...
waitUntil {
	...
	_dnTest = [_pos select 0, _pos select 1, (_pos select 2) - 0.05];
	_dir = getDir player;
	_goodTouch = false;
	_badTouch = false;
	_attachTo = objNull;
