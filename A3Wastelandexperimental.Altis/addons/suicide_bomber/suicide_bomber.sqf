...
_respawn_event_id = _bomber addEventHandler["Respawn", {[player] execVM "suicide_bomber_reset.sqf"}];
...
_setup_actions = {
	_perform_bombing_action = _bomber addAction["<t color='#FF0000'>Allahu Akbar</t>", "allahu_akbar.sqf"];
	_perform_deadmansswitch_action = _bomber addAction["<t color='#FF0000'>Dead Man's Switch</t>", "dead_mans_switch_exec.sqf"];
	...
	if(("allahuakbar" in _allVars) && ("deadmansswitch" in _allVars)) then {
		...
		if((!isNil "_release") && (!isNil "_cancel")) then {
			if((isNil "_release_action") && (isNil "_cancel_action")) then {
				...
				if("SatchelCharge_Remote_Mag" in (items _bomber + assignedItems _bomber + backpackitems _bomber + vestItems _bomber + uniformItems _bomber)|| "DemoCharge_Remote_Mag" in (items _bomber + assignedItems _bomber + backpackitems _bomber + vestItems _bomber + uniformItems _bomber)) then {
