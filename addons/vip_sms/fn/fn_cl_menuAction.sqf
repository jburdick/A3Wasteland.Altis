/*
	...
	Description: Perform one of the actions listed in the mine menu.
	
	Parameters:
		0: Control; control of current dialog from which to find the dialog's id (mine menu).
		1: Action; menu action to perform.

	Returns:
		Nothing.

	Example:
		[dummy] call vip_sms_fnc_cl_menuAction; //in most cases we use call
		[dummy] spawn vip_sms_fnc_cl_menuAction; //in some cases we spawn (typically when we're waiting on the result of a mine status change)
*/
...
if (_action < 8) then {
	...
	_nullDummy = (isNull _dummy) || ((player distance _dummy) > 2);
	
	if !(_nullDummy) then {

		_armed = if (_dummy isKindOf "Land_Map_F") then {_dummy getVariable "vip_mineArm"} else {(_dummy getVariable "vip_mineTripAttachedMine") getVariable "vip_mineArm"};
		...
		switch (_action) do {
			...
			case 0: { //disarm
				...
				if (_armed < 2) then {
					...
					if (_dummy isKindOf "Land_Map_F") then {
						[_dummy, 0] call vip_sms_fnc_cl_remove;
						[_control] call vip_sms_fnc_cl_menuUpdate;
					} else {
						_mine = _dummy getVariable "vip_mineTripAttachedMine";
						[_mine, 0] call vip_sms_fnc_cl_remove;
						waitUntil {isNull (_mine getVariable "vip_mineTripObj")};
