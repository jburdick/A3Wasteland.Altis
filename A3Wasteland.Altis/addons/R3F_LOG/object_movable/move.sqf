 ...
 * L'objet est relach� quand la variable R3F_LOG_joueur_deplace_object passe � objNull ce qui terminera le script
 * 
...
else
{
	...
	private ["_object", "_decharger", "_joueur", "_dir_joueur", "_arme_courante", "_muzzle_courant", "_mode_muzzle_courant", "_restaurer_arme","_IsProtected","_IsAllowed"];
	...
	private ["_action_release", "_action_aligner_pente", "_action_aligner_sol", "_action_aligner_horizon", "_action_tourner", "_action_rapprocher"];
	...
	if(_tempVar) exitwith {
		hint format["This object belongs to %1 and they're nearby you cannot take this.", _object getVariable "R3F_Side"]; R3F_LOG_mutex_local_verrou = false;
	};
	...
	if (isNull (_object getVariable ["R3F_LOG_est_transporte_par", objNull]) && (isNull (_object getVariable ["R3F_LOG_est_deplace_par", objNull]) || (!alive (_object getVariable ["R3F_LOG_est_deplace_par", objNull])) || (!isPlayer (_object getVariable ["R3F_LOG_est_deplace_par", objNull])))) then
	{
		if (isNull (_object getVariable ["R3F_LOG_remorque", objNull])) then
		{
			...
			{
				[_object, _joueur] call R3F_LOG_FNCT_define_owner_lock;
				
				_object setVariable ["R3F_LOG_est_deplace_par", _joueur, true];
				
				_joueur forceWalk true;
				
				R3F_LOG_joueur_deplace_object = _object;
				...
				if (_decharger) then
				{
					...
					_pos_rel_object_initial set [2, 0.1 + (_joueur selectionPosition "head" select 2) + (_pos_rel_object_initial select 1) * tan (89 min (-89 max (90-_elev_cam_initial)))];
				}
				else
				{
					R3F_LOG_deplace_dir_rel_object = (getDir _object) - _dir_joueur;
					
					_pos_rel_object_initial = _joueur worldToModel (_object modelToWorld [0,0,0]);
				...
				if (!local _object) then
				{
					...
					[_object, "setOwnerTo", _joueur] call R3F_LOG_FNCT_exec_commande_MP;
				...
				if (_object getVariable "R3F_LOG_est_deplace_par" == _joueur && isNull (_object getVariable ["R3F_LOG_est_transporte_par", objNull])) then
				{
					...
					_action_release = _joueur addAction [("<img image='client\icons\r3f_release.paa' color='#ee0000'/> <t color=""#ee0000"">" + format [STR_R3F_LOG_action_release_object, getText (configFile >> "CfgVehicles" >> (typeOf _object) >> "displayName")] + "</t>"), {_this call R3F_LOG_FNCT_object_release}, nil, 10, true, true];
					...
					_action_tourner = _joueur addAction [("<img image='client\icons\r3f_rotate.paa' color='#00eeff'/> <t color=""#00eeff"">" + STR_R3F_LOG_action_tourner + "</t>"), {R3F_LOG_deplace_dir_rel_object = R3F_LOG_deplace_dir_rel_object + 12; R3F_LOG_deplace_force_setVector = true;}, nil, 6, false, false];
					_action_rapprocher = _joueur addAction [("<img image='client\icons\r3f_rotate.paa' color='#00eeff'/> <t color=""#00eeff"">" + STR_R3F_LOG_action_rapprocher + "</t>"), {R3F_LOG_deplace_distance_rel_object = R3F_LOG_deplace_distance_rel_object - 0.4; R3F_LOG_deplace_force_attachTo = true;}, nil, 6, false, false];
					
					// Rel�cher l'objet d�s que le joueur tire. Le detach sert � rendre l'objet solide pour ne pas tirer au travers.
					_idx_eh_fired = _joueur addEventHandler ["Fired", {if (!surfaceIsWater getPos player) then {detach R3F_LOG_joueur_deplace_object; R3F_LOG_joueur_deplace_object = objNull;};}];
					...
					_object attachTo [_joueur, _pos_rel_object_initial];
					
					// Si �chec transfert local, mode d�grad� : on conserve la direction de l'objet par rapport au joueur
					if (!local _object) then {[_object, "setDir", R3F_LOG_deplace_dir_rel_object] call R3F_LOG_FNCT_exec_commande_MP;};
					
					R3F_LOG_mutex_local_verrou = false;
					
					// Boucle de gestion des �v�nements et du positionnement pendant le d�placement
					while {!isNull R3F_LOG_joueur_deplace_object && _object getVariable "R3F_LOG_est_deplace_par" == _joueur && alive _joueur} do
					{
						...
						if (local _object) then
						{
							...
							if (R3F_LOG_joueur_deplace_key_rotation == "X" || R3F_LOG_joueur_deplace_key_rotation == "C") then
							{
								...
								{
									if (R3F_LOG_joueur_deplace_key_rotation == "X") then {R3F_LOG_deplace_dir_rel_object = R3F_LOG_deplace_dir_rel_object + 4;};
									if (R3F_LOG_joueur_deplace_key_rotation == "C") then {R3F_LOG_deplace_dir_rel_object = R3F_LOG_deplace_dir_rel_object - 4;};
						...
						if (_object isKindOf "Static") then
						{
							...
							if (R3F_LOG_deplace_mode_alignement == "horizon") then
							{
								...
								_offset_hauteur_terrain_min = (_hauteur_terrain_min_max_object select 0) - (getPosASL _joueur select 2) + _offset_bounding_center;
								_offset_hauteur_terrain_max = (_hauteur_terrain_min_max_object select 1) - (getPosASL _joueur select 2) + _offset_bounding_center;
							...
							{
								_offset_hauteur_terrain_max = getTerrainHeightASL (getPos _object) - (getPosASL _joueur select 2) + _offset_bounding_center;
							...
							{
								_offset_hauteur = _offset_hauteur_terrain_min max (_offset_hauteur_terrain_max min ((_pos_rel_object select 2) + _offset_hauteur_cam)) + (getPosATL _joueur select 2);
							};
						}
						else
						{
							_offset_hauteur_terrain = getTerrainHeightASL (getPos _object) - (getPosASL _joueur select 2) + _offset_bounding_center;
					...
					if (isNull (_object getVariable ["R3F_LOG_est_transporte_par", objNull])) then
					{
						...
						[_object, "detachSetVelocity", [0, 0, 0.1]] call R3F_LOG_FNCT_exec_commande_MP;
					};
					...
					_joueur removeAction _action_release;
				...
				if (!isNull _object) then
				{
					...
					{
						R3F_LOG_PV_fin_deplacement_object = _object;
						publicVariable "R3F_LOG_PV_fin_deplacement_object";
						["R3F_LOG_PV_fin_deplacement_object", R3F_LOG_PV_fin_deplacement_object] call R3F_LOG_FNCT_PVEH_fin_deplacement_object;
					};
				};
			}
			else
	...
	if (local _object && {_object isKindOf "ReammoBox_F"}) then
	{
