...
_filesizeamountrandomizer = [163804,195072,206340,216340,226340];
...
T8_varTLine05 = "##  HACK ATM  ##";				// line for the addaction
	...
	[ _laptop, _id] spawn 
	{
		...
		while { !T8_varDiagAbort && alive player && (player getVariable ["FAR_isUnconscious", 0] == 0)} do
		{
			...
			if ( _newFile > T8_varFileSize ) then 
			{
				...
				_cashamountrandomizer = [10000,12000,15000,21000];
				...
				axeDiagLog = format ["%1 hacked ATM for %2 money", player, _cashamount];
