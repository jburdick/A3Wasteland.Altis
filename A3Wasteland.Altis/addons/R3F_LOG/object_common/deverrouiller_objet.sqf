...
else
{
	...
	private ["_object", "_duree", "_ctrl_titre", "_ctrl_fond", "_ctrl_jauge", "_time_debut", "_attente_valide", "_cursorTarget_distance"];
	...
	*/
	...
	while {_attente_valide && time - _time_debut < _duree} do
	{
		...
		if (!alive player || _cursorTarget_distance select 0 != _object || _cursorTarget_distance select 1 > 5) then
		{
