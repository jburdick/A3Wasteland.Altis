...
R3F_LOG_FNCT_3D_cursorTarget_distance_bbox =
{
	private ["_object", "_joueur"];
	
	_object = cursorTarget;
	_joueur = player;
	
	if (!isNull _object && !isNull _joueur && alive _joueur && cameraOn == _joueur) then
	{
		[
			_object,
			[
				_object worldToModel (_joueur modelToWorld (_joueur selectionPosition "head")),
				boundingBoxReal _object select 0,
				boundingBoxReal _object select 1
...
R3F_LOG_FNCT_3D_cursorTarget_virtuel =
{
	private ["_liste_ingores", "_distance_max", "_joueur", "_object_pointe", "_cursorTarget_distance"];
...
if (isNil "R3F_LOG_joueur_deplace_object") then {R3F_LOG_joueur_deplace_object = objNull};
addMissionEventHandler ["Draw3D",
{
	if !(isNull player) then
	{
		...
		if (!isNull R3F_LOG_joueur_deplace_object) then
		{
			//[R3F_LOG_joueur_deplace_object] call R3F_LOG_FNCT_3D_tracer_bbox_obj;
			
			{
				if ([
					R3F_LOG_joueur_deplace_object,
					boundingBoxReal R3F_LOG_joueur_deplace_object select 0,
					boundingBoxReal R3F_LOG_joueur_deplace_object select 1,
					_x,
					boundingBoxReal _x select 0,
					boundingBoxReal _x select 1
			...
			} forEach (nearestObjects [R3F_LOG_joueur_deplace_object, ["All"], 15] - [player, R3F_LOG_joueur_deplace_object]);
		}
		else
		{
		...
		//{
		//	[_x] call R3F_LOG_FNCT_3D_tracer_bbox_obj;
		//} forEach [[[R3F_LOG_joueur_deplace_object]] call R3F_LOG_FNCT_3D_cursorTarget_virtuel];
