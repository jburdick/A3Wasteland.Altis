/*
	...
	Description: Open the mine menu from scratch.
	
	Parameters:
		0: Dialog; dialog of mine menu.

	Returns:
		Nothing.

	Example:
		[dialog] call vip_sms_fnc_cl_menuOpen;
*/
...
_tempListMines = nearestObjects [player, ["Land_Map_F"], 2];
_tempListTrips = nearestObjects [player, ["Land_CanOpener_F"], 2];
_tempList = [_tempListMines, _tempListTrips] call BIS_fnc_arrayPushStack; //we were promised the append command in 1.40, yet it doesn't work...

{
	if (!(isNull (_x getVariable ["vip_mineObj", objNull])) || !(isNull (_x getVariable ["vip_mineTripAttachedMine", objNull]))) then {
...
if ((count vip_sms_var_cl_mineList) > 0) then {
	...
	
	{
		...
		if (_x isKindOf "Land_CanOpener_F") then {
			_count8 = _count8 + 1;
			_type = ["Tripwire Peg", _count8];
		};
...
(_dialog displayCtrl 22) ctrlSetText ""; //status text
