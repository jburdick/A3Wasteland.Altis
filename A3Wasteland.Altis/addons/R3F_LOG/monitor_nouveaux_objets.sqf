/**
 * Recherche p�riodiquement les new objets pour leur ajouter les fonctionnalit�s de logistique si besoin
...
private
[
	"_compteur_cyclique", "_liste_new_objects", "_liste_vehicules_connus", "_liste_statiques", "_liste_new_statiques",
	"_liste_statiques_connus", 	"_liste_statiques_cycle_precedent", "_count_liste_objects", "_i", "_object", "_fonctionnalites",
...
while {true} do
{
	...
	{
		// Tout les __nb_cycles_commande_vehicles ou sur ordre, on r�cup�re les new v�hicules du jeu
		if (_compteur_cyclique == 0 || R3F_LOG_PUBVAR_nouvel_object_a_initialiser) then
		{
			R3F_LOG_PUBVAR_nouvel_object_a_initialiser = false; // Acquittement local
			...
			// R�cup�ration des new v�hicules
			_liste_new_objects = vehicles - _liste_vehicules_connus;
			_liste_vehicules_connus = _liste_vehicules_connus + _liste_new_objects;
		}
		else
		{
			_liste_new_objects = [];
		...
		// En plus des new v�hicules, on r�cup�re les statiques (caisse de mun, drapeau, ...) proches du joueur non connus
		...
		{
			_liste_new_statiques = _liste_statiques - _liste_statiques_cycle_precedent - _liste_statiques_connus;
			_liste_statiques_connus = _liste_statiques_connus + _liste_new_statiques;
			_liste_statiques_cycle_precedent = _liste_statiques;
		}
		else
		{
			_liste_new_statiques = [];
			_liste_statiques_cycle_precedent = [];
		};
		
		_liste_new_objects = _liste_new_objects + _liste_new_statiques;
		_count_liste_objects = count _liste_new_objects;
		
		if (_count_liste_objects > 0) then
		{
			// On parcoure tous les new objets en __tempo secondes
			for [{_i = 0}, {_i < _count_liste_objects}, {_i = _i + 1}] do
			{
				_object = _liste_new_objects select _i;
				_fonctionnalites = [typeOf _object] call R3F_LOG_FNCT_determiner_fonctionnalites_logistique;
				...
				{
					_object setVariable ["R3F_LOG_fonctionnalites", _fonctionnalites, false];
					
					if (isNil {_object getVariable "R3F_LOG_disabled"}) then
					{
						_object setVariable ["R3F_LOG_disabled", R3F_LOG_CFG_disabled_by_default, false];
					};
				...
				if (_object getVariable ["R3F_LOG_CF_depuis_usine", false]) then
				{
					_object addAction [("<t color=""#ff9600"">" + format [STR_R3F_LOG_action_revendre_usine_direct, getText (configFile >> "CfgVehicles" >> (typeOf _object) >> "displayName")] + "</t>"), {_this call R3F_LOG_FNCT_usine_revendre_direct}, nil, 5, false, true, "", "!R3F_LOG_mutex_local_verrou && R3F_LOG_object_addAction == _target && R3F_LOG_action_revendre_usine_direct_valide"];
				};
				
				sleep (0.07 max (__tempo / _count_liste_objects));
			};
		}
		else
		{
