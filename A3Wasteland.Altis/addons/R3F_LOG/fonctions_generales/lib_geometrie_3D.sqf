 ...
 *
 * // Utilitaires
 * R3F_LOG_FNCT_3D_cursorObject_distance_bbox
 * R3F_LOG_FNCT_3D_cursorObject_virtuel
...
R3F_LOG_FNCT_3D_cursorObject_virtuel =
{
	private ["_liste_ingores", "_distance_max", "_joueur", "_objet_pointe", "_cursorObject_distance"];
	...
	if (!isNull (_cursorObject_distance select 0) &&
		{!((_cursorObject_distance select 0) in _liste_ingores) && (_cursorObject_distance select 1) <= _distance_max}
	) then
	{
...
addMissionEventHandler ["Draw3D",
{
	if !(isNull player) then
	{
		...
		else
		{
			...
			{
				_cursorObject_distance = call R3F_LOG_FNCT_3D_cursorObject_distance_bbox;
				hintSilent format ["%1 | %2", typeOf (_cursorObject_distance select 0), _cursorObject_distance select 1];

				if !(isNull (_cursorObject_distance select 0)) then
				{
					[_cursorObject_distance select 0] call R3F_LOG_FNCT_3D_tracer_bbox_obj;
				};
			};
		};
		...
		//{
		//	[_x] call R3F_LOG_FNCT_3D_tracer_bbox_obj;
		//} forEach [[[R3F_LOG_joueur_deplace_objet]] call R3F_LOG_FNCT_3D_cursorObject_virtuel];
